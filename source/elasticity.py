...
def isoev(K):
    ...
    '''
    return np.array([
            ten.sp(K,P1)/ten.sp(P1,P1)
            ,ten.sp(K,P2)/ten.sp(P2,P2)
            ])
...
def plot2phiso(K,n=20,vl=0):
    ...
    for i in [0,3]:
        ...
        plt.plot(v1,hsupp[:,i,i],color='lightgray',linestyle='-',label='HS_upp')
        plt.plot(v1,hslow[:,i,i],color='lightgray',linestyle='--',label='HS_low')
